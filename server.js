...
function requireAdmin(req, res, next) {
  if (req.session.role !== ROLES.ADMIN) {
    return res
      .status(403)
      .json({ success: false, message: "權限不足 (Forbidden)" });
  }
  next();
}
...
app.post("/api/login", (req, res) => {
  ...
  if (password !== ADMIN_PASSWORD) {
    return res
      .status(401)
      .json({ success: false, message: "密碼錯誤 (Incorrect password)" });
  }
  ...
  if (
    ...
    req.session.votedOption &&
    votes[req.session.votedOption] !== undefined
  ) {
...
app.post("/api/vote", validateVotingRound, (req, res) => {
  ...
  if (req.session.role === ROLES.ADMIN) {
    return res
      ...
      .json({
        success: false,
        message: "管理員無法投票，僅能觀看結果 (Admins cannot vote)",
      });
  }
  ...
  if (activeRequests.has(req.sessionID)) {
    return res
      ...
      .json({
        success: false,
        message: "處理中，請稍候 (Processing, please wait)",
      });
  }
  ...
  try {
    ...
    if (votes[option] === undefined) {
      return res
        .status(400)
        .json({ success: false, message: "無效的選項 (Invalid option)" });
    }
